...
def update_feeds():
    ...
    if False:#now.hour >= 22 or now.hour < 6 or now.weekday() in [5, 6]:
        print(
...
def add_ai_summary(article_obj_lst):
    """Use YANDEX_GPT's ChatGPT API to get artcile summaries"""
    if settings.YANDEX_GPT is None:
        print("Not Requesting AI article summaries as YANDEX_GPT not set.")
    else:
        ...
        config = {
            ...
            "catalog_id": settings.YANDEX_GPT_FOLDER_ID,
            "iam_token": settings.YANDEX_GPT
        }
        ...
        for article_obj in article_obj_lst:
            ...
            try:
                ...
                completion = yandex_gpt.get_sync_completion(
                    
                    messages=[
                        {
                            "role": "system",
                            "content": f"Summarize this article in {bullets} bulletpoints. write only in Russian",
                        },
                        {
                            "role": "user",
